...
from backend.auth.auth import create_jwt_token
from backend.services.user_service import register_new_user, authenticate_user
...
async def user_register(user: RegisterUser, db: Session = Depends(get_db)):
    """Register a new user."""
    try:
        new_user = await register_new_user(db, user)
...
async def user_login(
    ...
    try:
        ...
        jwt_token = await create_jwt_token(data=user)
